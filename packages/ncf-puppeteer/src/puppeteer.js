...
const ENV_VARS = { XVFB: null, HEADLESS: '1', CHROME_EXECUTABLE_PATH: '' };
const log = { info: () => {}, error: console.error, warn: console.warn };
    ...
    anonymizeProxy(opts.proxyUrl)
      ...
      .then(browser => {
        const cleanUp = () => {
          ...
          closeAnonymizedProxy(anonymizedProxyUrl, true).catch(err => {
            console.log(
              `WARNING: closeAnonymizedProxy() failed with: ${err.stack || err}`
            );
          });
        };
...
const getPuppeteerOrThrow = () => {
  try {
    ...
    if (err.code === 'MODULE_NOT_FOUND')
      err.message =
        "Cannot find module 'puppeteer'. Did you choose the correct base Docker image?";
...
const launchPuppeteer = (opts = {}) => {
  ...
  if (optsCopy.headless === undefined || optsCopy.headless === null) {
    optsCopy.headless =
      ...
      process.env[ENV_VARS.XVFB] !== '1';
  }
  if (
    optsCopy.useChrome &&
    (optsCopy.executablePath === undefined || optsCopy.executablePath === null)
  ) {
    optsCopy.executablePath =
      process.env[ENV_VARS.CHROME_EXECUTABLE_PATH] ||
